...
Origin: https://github.com/jwngr/sdow/blob/master/sdow/database.py
"""

import os.path
import sqlite3
import helpers as helpers
from wikipedia_breadth_first_search import breadth_first_search
...
class Database(object):
    ...
    def get_root_id(self, page_id):
        query = 'SELECT is_redirect FROM pages WHERE id = ?;'
        query_bindings = (page_id,)
        self.sdow_cursor.execute(query, query_bindings)

        is_redirect = bool(self.sdow_cursor.fetchone()[0])
        if not is_redirect:
            return page_id
        else:
            ...
            query_bindings = (page_id,)
            self.sdow_cursor.execute(query, query_bindings)
            return self.get_root_id(self.sdow_cursor.fetchone()[0])
    ...
    Returns:
      ...
      :param incoming_or_outgoing_links_count:
    """
        ...
        # fetch incoming links count zle działa
        self.sdow_cursor.execute(query)

        cursor = self.sdow_cursor.fetchone()
