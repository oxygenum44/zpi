...
for i in range(2, 20):
    ...
    elbow_wart.append(analyze.silhoutte(tweet_features_clusters))
    print(elbow_wart)
    fig = plt.figure()
    for iii in range(len(tweet_features_clusters)):
        ax = fig.add_subplot(1, len(tweet_features_clusters), iii + 1,
                             title=str(len(processed_tweets_clusters[iii])) + ", " + cluster_names_two_words_tf_idf[
                                 iii])
