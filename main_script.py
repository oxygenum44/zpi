...
clustering = models.TweetsKMeans(processedTweets, 5, 'tf_idf')
#centroids, assigned = clustering.run_k_means(20, 'euclidean')
centroids, assigned = clustering.run_k_means(20)
text_clusters = models.group_tweets(processedTweets, assigned, 5)
...
for i in range(len(text_clusters)):
    print('KLASTER '+cluster_names_one_word[i]+", "+cluster_names_one_word_tf_idf[i])
    for j in range(len(text_clusters[i])):
        print(text_clusters[i][j])

fig = plt.figure()
for i in range(len(text_clusters)):
    ax = fig.add_subplot(1,len(text_clusters),i+1, title=i)
